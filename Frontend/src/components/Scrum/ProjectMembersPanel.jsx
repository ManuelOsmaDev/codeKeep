...
import { X, Users, Crown, Shield, User, Trash2, Loader2, AlertTriangle } from 'lucide-react';
...
const ProjectMembersPanel = ({ isOpen, onClose, projectId, onMembersChange }) => {
    ...
    const initiateRemoveMember = (userId, userName) => {
        setConfirmModal({ isOpen: true, userId, userName });
    };

    const proceedWithRemove = async () => {
        ...
        try {
            ...
            // Show custom error modal if message comes from backend
            const responseData = error.response?.data;
            const msg = responseData?.message || 'Failed to remove member';
            // Check for structured activity list
            const activities = responseData?.assignedActivities || [];

            setErrorModal({ isOpen: true, message: msg, activities });
        }
    };
        ...
        <>
                                ...
                                {members.map((member) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            {member.role !== 'owner' && (
                                                <button
                                                    onClick={() => initiateRemoveMember(member.userId, member.user?.name)}
            ...
            {confirmModal.isOpen && (
                <div className="fixed inset-0 z-[60] flex items-start justify-center pt-20 bg-black/20 backdrop-blur-[1px]">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 transform transition-all scale-100 mx-4 border border-slate-100 dark:border-slate-700">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 mb-6 text-sm">
                                Are you sure you want to remove <span className="font-semibold text-slate-700 dark:text-slate-300">{confirmModal.userName}</span> from this project?
                            </p>
                            <div className="flex gap-3 w-full">
                                <button
                                    onClick={() => setConfirmModal({ isOpen: false, userId: null, userName: '' })}
                                    className="flex-1 px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors font-medium text-sm"
                                >
                                    Cancel
                                </button>
                                <button
            ...
            {/* Error Modal (for dependencies) */}
            {errorModal.isOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 mx-4 border border-slate-100 dark:border-slate-700">
                        <div className="flex flex-col items-center text-center">
                            <div className="w-12 h-12 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center mb-4">
                                <AlertTriangle className="w-6 h-6" />
                            </div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400 mb-6 text-sm">
                                {errorModal.message}
                                {errorModal.activities && errorModal.activities.length > 0 && (
                                    <span className="font-bold text-slate-700 dark:text-slate-200">
                                        {' ' + errorModal.activities.join(', ')}
                                    </span>
                                )}
                            </p>
                            <button
                                onClick={() => setErrorModal({ isOpen: false, message: '', activities: [] })}
                                className="w-full px-4 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors font-semibold shadow-lg shadow-indigo-200 dark:shadow-none"
                            >
                                Understood
                            </button>
                        </div>
